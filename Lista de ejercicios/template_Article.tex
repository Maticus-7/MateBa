...
% Título y autor
\title{\textbf{Lista de ejercicios}}
\author{Profesor: Matihus A. Molina Larios}
\date{\today}
...
\begin{document}
	...
	\begin{definition}[Conectores lógicos]
		\begin{itemize}
			\item Negación \(\neg p\)
			\item Conjunción \(p \land q\) (``y'')
			\item Disyunción \(p \lor q\) (``o'' inclusiva)
			\item Disyunción exclusiva \(p \lor q\) (``o \ldots o \ldots'')
			\item Condicional \(p \to q\) (``si \ldots entonces \ldots'')
			\item Bicondicional \(p \leftrightarrow q\) (``si y solo si'')
		\end{itemize}
	\end{definition}
	
	\begin{definition}[Proposiciones relacionadas a la condicional]
		\begin{itemize}
			\item Recíproca: \(q \to p\)
			\item Inversa: \(\neg p \to \neg q\)
			\item Contrapositiva: \(\neg q \to \neg p\)
		\end{itemize}
	\end{definition}
	...
	\begin{definition}[XD]
		\begin{itemize}
			\item \textbf{Tautología}: Proposición siempre verdadera.
			
			\item \textbf{Contradicción}: Proposición siempre falsa.
			
			\item \textbf{Equivalencia lógica} \(P \equiv Q\): Misma tabla de verdad.
		\end{itemize}
	\end{definition}
	
	\begin{definition}[Leyes de De Morgan]
		\[
		\neg(p \lor q) \equiv \neg p \land \neg q \quad \text{y} \quad \neg(p \land q) \equiv \neg p \lor \neg q
		\]
	\end{definition}
	
	\begin{definition}[Cuantificadores]
		\begin{itemize}
			\item Universal \(\forall\) (``para todo'')
			\item Existencial \(\exists\) (``existe al menos uno'')
			\item Existencial único \(\exists!\) (``existe solo uno'')
		\end{itemize}
	\end{definition}
	...
	\subsection{Ejemplos Interesantes}
